...
const mathematicsChapters = d2d.filter((item) => item.subject === 'Mathematics');

// Get unique values for filters
const classes = [...new Set(mathematicsChapters.map((chapter) => chapter.class))];
const units = [...new Set(mathematicsChapters.map((chapter) => chapter.unit))];
// const statuses = [...new Set(mathematicsChapters.map((chapter) => chapter.status))];
...
export default function MathematicsPage() {
  ...
  const filteredChapters = useMemo(() => {
    let chapters = mathematicsChapters.filter((chapter) => {
    ...
    <div className="overflow-y-auto bg-white dark:bg-[#181F2A] min-h-screen w-full px-2 py-2 md:px-8 md:py-6 m-0">
      ...
      <div className="hidden md:flex md:items-center md:justify-center md:mb-1 w-full">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold text-gray-900 dark:text-[#E2E8F0]">Mathematics PYQs</h1>
        </div>
      </div>
      <p className="md:text-sm md:text-center md:text-gray-500 md:mt-3 md:mb-5 hidden md:block dark:text-[#E2E8F0]">Chapter-wise Collection of Mathematics PYQs</p>
      ...
      <div className='flex flex-row justify-between gap-4 pb-4 pt-2 md:mb-2 px-4 md:px-0'>
        <div className="text-[13px] md:text-sm text-gray-500 dark:text-[#E2E8F0]">
          Showing {filteredChapters.length} of {mathematicsChapters.length} chapters
        </div>
        {/* Sort dropdown  */}
        <DropdownMenu>
